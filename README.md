<!--
  _____________________________________________________________
  |  HELLO THERE! Welcome to my GitHub Profile.               |
  |  This README acts as my interactive CV.                  |
  |  Feel free to connect or collaborate!                    |
  |___________________________________________________________|
-->

<p align="center">
  <!-- ANIMATED BANNER PLACEHOLDER -->
  <!-- Replace the src below with your own banner GIF or image -->
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&duration=3000&pause=1000&color=00D2FF&center=true&vCenter=true&width=700&lines=Hi%2C+I'm+Sirgae+Youfski;Backend+Developer+%26+Full+stack+Developer+%26+Data+Analyst;Turning+Data+into+Insights;ETL+%7C+APIs+%7C+Python" alt="Animated Typing Banner" />
</p>

---

### 👨‍💻 About Me

I am a **Backend Developer & Data Analyst** with a strong engineering mindset and a passion for building scalable data pipelines and robust RESTful APIs. I love transforming raw, chaotic data into clear, actionable insights.

- 🔭 I’m currently working on **Comprehensive NBA Data Analysis** (Web Scraping + SQL + Stats).
- 🌱 I’m currently learning advanced **Machine Learning** and **Big Data** technologies.
- 👯 I’m looking to collaborate on open-source **Data Analytics** and **Python Backend** projects.
- 🎓 Currently pursuing my **Bachelor's in Computer Engineering** at Azad University.
- 💬 Ask me about **Python, Django, FastAPI, Web Scraping, or SQL**.
- ⚡ Fun fact: I was a certified Python instructor at Fasa University and an editor recognized by the Ministry of Science!

---

### 🛠️ Tech Stack & Tools

#### 💻 Programming Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)

#### 🌐 Web Frameworks
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)

#### 📊 Data & Analytics
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![BeautifulSoup](https://img.shields.io/badge/BeautifulSoup-4B8BBE?style=for-the-badge&logo=python&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)

#### 🗄️ Databases
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=for-the-badge&logo=sqlite&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)
![ClickHouse](https://img.shields.io/badge/ClickHouse-FFCC01?style=for-the-badge&logo=clickhouse&logoColor=black)

#### 🧰 Other Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)
![Metabase](https://img.shields.io/badge/Metabase-509EE3?style=for-the-badge&logo=metabase&logoColor=white)
![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)

---

### 💼 Featured Projects & Experience

#### 🏀 **Comprehensive NBA Data Analysis** *(Quera Bootcamp - Jul 2026 - Present)*
- Built an end-to-end data pipeline: Web Scraping (`BeautifulSoup4`, `Selenium`) ➡️ Database Storage (`SQL`) ➡️ Statistical Analysis (`Pandas`, `IPython`).
- Performed complex hypothesis testing on professional NBA player performance data from Basketball-Reference.com.

<!-- PROJECT DEMO GIF PLACEHOLDER -->
<p align="center">
  <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExY3M3ZGozN3RweWxwcDVpdnJ2eHpmcWZpMmp3MHpnM2dxeHozY3c2cSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/LMt9638dO8dftA5BZz/giphy.gif" width="400" alt="Data Analysis Animation">
  <br>
  <em>Figure: Dynamic data scraping and analytics workflow (Placeholder)</em>
</p>

#### 🧑‍🏫 **Python Instructor** *(Fasa University - Dec 2023 to Feb 2024)*
- Taught introductory and intermediate Python programming to university students.
- Designed practical coding exercises, graded assignments, and received outstanding positive feedback from the faculty.

#### 📝 **Editor - Computer Science Association** *(Fasa University - Nov 2023)*
- Led the editorial team for the department's newsletter and academic publications.
- Recognized by the **Ministry of Science** for outstanding contributions to the academic community.

---

### 🎓 Education

| Degree | Field | Institution | Duration | GPA |
| :--- | :--- | :--- | :--- | :--- |
| **Bachelor of Science** | Computer Engineering | Islamic Azad University | Sep 2025 - Present | - |
| **Associate of Science** | Computer Science | Fasa University | Sep 2022 - Feb 2024 | **17.5/20** |

---

### 📜 Certifications & Awards

- 🏅 **Data Analysis Bootcamp** - Quera (2026)
- 🏅 **Certificate of Appreciation (BioInformatics)** - Faculty of Fasa University (Mar 2024)
- 🏅 **Python Instructor Certificate** - Fasa University (Feb 2023)
- 🏅 **Honoured Editor** - Ministry of Science (Nov 2023)

---

### 🌐 Languages

| Language | Proficiency |
| :--- | :--- |
| **Persian** | Native |
| **English** | Intermediate |
| **Russian** | Beginner |

---

### 📊 GitHub Analytics

<p align="center">
  <!-- Animated GitHub Stats Cards -->
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img height="180em" src="https://github-readme-stats.vercel.app/api?username=GoldenIsland00&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&count_private=true" />
  </a>
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=GoldenIsland00&layout=compact&theme=radical&hide_border=true&bg_color=0D1117" />
  </a>
</p>

---

### 🔥 Contribution Snake (Animated)

<!-- The famous animated snake eating your contributions. You need to set up a GitHub Action to generate this automatically, or just comment it out for now. -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/GoldenIsland00/GoldenIsland00/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/GoldenIsland00/GoldenIsland00/output/github-contribution-grid-snake.svg" />
  <img alt="github-snake" src="https://raw.githubusercontent.com/GoldenIsland00/GoldenIsland00/output/github-contribution-grid-snake-dark.svg" />
</picture>

> *💡 Tip: To enable the snake animation, create a `.github/workflows/snake.yml` file in your repo to run the generation script.*

---

### 🤝 Let's Connect!

I'm always open to collaborating on interesting projects or discussing new technologies.

<p align="center">
  <a href="mailto:sirgae.youfski@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/LiamTaghizadeh">
    <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="#">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://t.me/yourtelegram">
    <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" />
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=LiamTaghizadeh&label=Profile%20Views&color=0e75b6&style=flat" alt="Profile Visitor Counter" />
</p>

---

⭐ **Thanks for stopping by!** Feel free to star ⭐ this repository if you liked it, and don't forget to follow me for more awesome projects!
